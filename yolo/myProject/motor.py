...
class motorCtrl:
    def __init__(self, motor_id, postion: float, maxAngles:float):
        self.ID = np.uint8(motor_id)
        self.info = motorInformation(motor_id, maxAngles)
        self.bootPosition(postion)
                        ...
                        if r[i + 2] == self.ID:
                            ...
                            if self.info.angle > self.info.maxAngles and self.info.angle < 360.0 - self.info.maxAngles:
                                ...
                                if self.info.angle > 360.0-self.info.maxAngles:
                                    self.info.angle = self.info.angle - 360.0
                                return self.info.encoder, self.info.angle
